...
!function(window, document) {
    function initOptionValues() {
        DEBUG = null == Options.debug ? 1 : Options.debug, IS_REMOVE_ATTR_BIND = null == Options.removeAttrDataBind ? 1 : Options.removeAttrDataBind, 
        DOM_ATTR_BIND = Options.domAttrBind || (Options.domAttrBind = "data-bind"), SELECTOR_DATA_BIND = S_LEFT_ZKH + DOM_ATTR_BIND + S_RIGHT_ZKH, 
        DATA_KEY_FN_ID = Options.dataKeyFnId || (Options.dataKeyFnId = "_$id"), DATA_KEY_FN_PARENT = Options.dataKeyFnParent || (Options.dataKeyFnParent = "_$parent"), 
        ...
        UID_PREFIX_DATA = Options.uidPrefixData || (Options.uidPrefixData = "_D"), UID_PREFIX_TEMPLATE = Options.uidPrefixTemplate || (Options.uidPrefixTemplate = "_T");
    }
    ...
    function set(data, key, value, forceUpdate) {
        ...
        if (forceUpdate || isObject(value) || value !== oldValue) if (isObject(oldValue) && (once(EVENT_CHECK_MEMORY, checkMapDataIdDataObject), 
        notify(2e3, EVENT_CHECK_MEMORY)), observe(value, data, key), data[key] = value, 
        trigger(EVENT_DATA_CHAGE, data, key, value, oldValue), isArray(data)) {
    ...
    function checkMapDataIdDataObject() {
        var keys = [];
        for (var k in _mapDataIdDataObject) keys.push(k);
        for (var i = 0; i < keys.length; i++) checkMapDataId(keys[i]);
    }
    function checkMapDataId(dataid) {
        async(function() {
            document.querySelector("." + dataid) || delete _mapDataIdDataObject[dataid];
        }, 100);
    }
    ...
    function getElementBindInfo(el, data, cssFields) {
        ...
        if (bindInfo[S_BIND_INFO_PROP_DATA_ID] = getDataId(data), cssFields) for (var key in bindInfo) if (key != S_BIND_INFO_PROP_DATA_ID && key != BIND_KEY_FIELD && key != BIND_KEY_CLICK) if (key == BIND_KEY_STYLE) for (var kv, kvs = bindInfo[key].split(";"), i = 0; i < kvs.length; i++) {
            kv = kvs[i].split("=");
            var fields = [];
            getBindValue(data, kv[1], fields);
            for (var j = 0; j < fields.length; j++) cssFields.push(fields[j]);
        } else if (key == BIND_KEY_CLASS) for (var kv, kvs = bindInfo[key].split(";"), i = 0; i < kvs.length; i++) if (kv = kvs[i].split("="), 
        1 == kv.length) {
            var fields = [];
            getBindValue(data, kv[0], fields);
            for (var j = 0; j < fields.length; j++) cssFields.push(fields[j]);
        } else {
            var fields = [];
            getBindValue(data, kv[1], fields);
    ...
    function defineElement(el, data) {
        var cssFields = [], bindInfo = getElementBindInfo(el, data, cssFields), dataid = getDataId(data);
        if (dataid && addClass(el, dataid), each(cssFields, function(css) {
            addClass(el, css);
    ...
    function parseTemplate(dom, selector) {
        ...
        each(els, function(el) {
            ...
            if (hasTemplate) {
                ...
                setAttr(el, S_ELEMENT_ATTR_TEMPLATE, templateid), document[templateid] = template, 
                removeChilds(el);
            }
        });
    }
    ...
    function installRenders() {
        _renderMap || (_renderMap = {}, putRender("*", function(el, prop, val) {
            ...
            if (data[bindText]) return void setClass(el, data[bindText]);
            var keys = [], txts = [];
    ...
    function createFragmentByTemplate(data, el, foreach) {
        ...
        })) : (rs = document[templateid].cloneNode(!0), each(rs.querySelectorAll(SELECTOR_DATA_BIND), function(node) {
            defineElement(node, data), elementRender(node);
        })), removeDataBindAttr(rs), rs;
    }
    function elementRender(el, bindInfo, dataField) {
        ...
        each(_renders, function(render) {
            ...
            if (bindText) {
                ...
                if (render[S_RENDER_EVENT]) addEvent(render[S_RENDER_EVENT], functionOfBind, el); else {
                    var data = getData(bindInfo[S_BIND_INFO_PROP_DATA_ID]);
                    render[S_RENDER_FN](el, data, bindText);
                }
            }
        });
        for (var bindKey in bindInfo) if (!_renderMap[bindKey] && bindKey != S_BIND_INFO_PROP_DATA_ID && bindKey != S_BIND_INFO_PROP_TEMPLATE_ID) {
    ...
    function removeDataBindAttr(dom) {
        if (IS_REMOVE_ATTR_BIND) {
            var els = (dom || document).querySelectorAll(SELECTOR_DATA_BIND);
            each(els, function(el) {
                el.removeAttribute(DOM_ATTR_BIND);
            });
        }
    }
    ...
    function addClass(el, name) {
        if (name) if ($isIE) if (el.className) {
    ...
    function setClass(el, cls) {
        var addNames = cls.split(" ");
        if ($isIE) el.className = ""; else {
            for (var delNames = [], i = 0; i < el.classList.length; i++) startsWith(el.classList[i], UID_PREFIX_DATA) || delNames.push(el.classList[i]);
            for (var i = 0; i < delNames.length; i++) el.classList.remove(delNames[i]);
        }
        for (var i = 0; i < addNames.length; i++) addClass(el, addNames[i]);
    }
    ...
    function on(eventNames, fn) {
        eventNames && isFunction(fn) && eventNames.replace(REG_EVENT, function(name, pos) {
            for (var isExist = !1, fns = _callbacks[name] = _callbacks[name] || [], i = 0; i < fns.length; i++) if (fns[i] == fn) {
                isExist = !0;
                break;
            }
            !isExist && fns.push(fn);
    ...
    function once(eventNames, fn) {
        fn.once = 1, on(eventNames, fn);
    }
    ...
    function trigger(eventNames, eventObj) {
        ...
        eventNames.replace(REG_EVENT, function(name) {
            for (var fns = _callbacks[name] || [], i = 0; i < fns.length; i++) {
                var fn = fns[i];
                fn.once && off(name, fn), tryApply(eventObj, fn, args);
            }
        });
    }
    ...
    function bind(data, selector, opt) {
        ...
        addEvent("change", datachangeEventListener, document), removeDataBindAttr(dom);
    }
    ...
    var DEBUG, IS_REMOVE_ATTR_BIND, DOM_ATTR_BIND, SELECTOR_DATA_BIND, DATA_KEY_FN_ID, DATA_KEY_FN_PARENT, DATA_KEY_FN_ROOT, DATA_KEY_FN_DATA, BIND_REF_ROOT, BIND_REF_PARENT, BIND_REF_DATA, BIND_KEY_FIELD, BIND_KEY_VALUE, BIND_KEY_INNERTEXT, BIND_KEY_INNERHTML, BIND_KEY_OPTIONS, BIND_KEY_READONLY, BIND_KEY_DISABLED, BIND_KEY_VISIBLE, BIND_KEY_CHECKED, BIND_KEY_STYLE, BIND_KEY_CLASS, BIND_KEY_FOREACH, BIND_KEY_WITH, BIND_KEY_IF, BIND_KEY_CLICK, EVENT_DATA_CHAGE, EVENT_UPDATE_VIEW, UID_PREFIX_DATA, UID_PREFIX_TEMPLATE, Options = {}, S_BIND_INFO_PROP_DATA_ID = "$d", S_BIND_INFO_PROP_TEMPLATE_ID = "$t";
    initOptionValues();
    var _renderMap, _renders, slice = Array.prototype.slice, async = setTimeout, _mapDataIdDataObject = {}, S_REG_QUOTE = /(['"])[^'"]*\1/g, S_REG_PROP = /([\w$_\.\[\]]+)/g, _fns = {}, S_ELEMENT_PROP_BIND_INFO = "_bindInfo", S_ELEMENT_ATTR_TEMPLATE = "_template", EVENT_CHECK_MEMORY = "_eventCheckMap", S_IGNORE_KEY = "true false null alert this _ if else".split(" "), S_RENDER_KEY = "k", S_RENDER_FN = "f", S_RENDER_ORDER = "o", S_RENDER_TEMPLATE = "t", S_RENDER_EVENT = "e", REG_EVENT = /\S+/g, _callbacks = {};
