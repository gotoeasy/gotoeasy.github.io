...
(function($$, escapeHtml) {
    // 组件注册
    rpose.registerComponents({ "ui-button": UiButton, "demo-3-4-button": Demo34Button });
    ...
    // 属性接口定义
    Demo34Button.prototype.$OPTION_KEYS = null; // 可通过标签配置的属性，未定义则不支持外部配置
    Demo34Button.prototype.$STATE_KEYS = null; // 可更新的state属性，未定义则不支持外部更新state

    // 组件函数
    function Demo34Button(options = {}) {
        // 组件默认选项值
        this.$options = {};
     ...
     * 节点模板函数
     */
    Demo34Button.prototype.nodeTemplate = function nodeTemplate($state, $options, $actions, $this) {
        return {
            r: 1,
            t: "div",
            a: { class: { _1rj337o: 1 } },
            c: [
                {
                    ...
                    c: [
                        { s: "\r\n        ", k: 3 },
                        { t: "li", c: [{ s: escapeHtml($state.msg), k: 5 }], x: 1, k: 4 },
                        { t: "li", c: [{ t: "ui-button", a: { onclick: $actions.btn1Click, value: "按钮1" }, m: 1, k: 7 }], x: 1, k: 6 },
                        { t: "li", c: [{ t: "ui-button", a: { onclick: $actions.btn2Click, value: "按钮2" }, m: 1, k: 9 }], x: 1, k: 8 },
                        {
                            t: "li",
                            c: [{ t: "ui-button", a: { onclick: $actions.btn3Click, ref: "btn3", value: "模拟重试", $context: $this }, m: 1, k: 11 }],
                            x: 1,
                            k: 10
                        }
                    ],
    ...
    // 组件挂载
    rpose.mount(rpose.newComponentProxy("demo-3-4-button").render(), "body");
