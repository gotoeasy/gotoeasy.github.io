            ...
            function(require, module, exports) {
                ...
                (function($$) {
                    ...
                    rpose.registerComponents({
                        "demo-slot-component-default": DemoSlotComponentDefault,
                        "p102-demo-helloworld-p2d2": P102DemoHelloworldP2d2,
                        "p103-demo-component-p3d4": P103DemoComponentP3d4,
                        "p106-demo-slot-p6d2": P106DemoSlotP6d2,
                        router: $rpose$buildin$Router,
                        "ui-button": UiButton,
                        "ui-helloworld": UiHelloworld,
                        "p107-demo-route-p7d4": P107DemoRouteP7d4
                    }); // ------------------------------------------------------------------------------------------------------
                    // 组件 DemoSlotComponentDefault
                    // 注:应通过rpose.newComponentProxy方法创建组件代理对象后使用，而不是直接调用方法或用new创建
                    // ------------------------------------------------------------------------------------------------------
                    // 属性接口定义

                    DemoSlotComponentDefault.prototype.$OPTION_KEYS = undefined; // 可通过标签配置的属性，未定义则不支持外部配置

                    DemoSlotComponentDefault.prototype.$STATE_KEYS = ["$SLOT"]; // 可更新的state属性，未定义则不支持外部更新state
                    // 组件函数

                    function DemoSlotComponentDefault() {
                    ...
                    DemoSlotComponentDefault.prototype.nodeTemplate = function nodeTemplate($state, $options, $actions, $this) {
                        var v_Array = [];
                    ...
                    // 组件 P103DemoComponentP3d4
                    // 注:应通过rpose.newComponentProxy方法创建组件代理对象后使用，而不是直接调用方法或用new创建
                    // ------------------------------------------------------------------------------------------------------
                    // 属性接口定义

                    P103DemoComponentP3d4.prototype.$OPTION_KEYS = undefined; // 可通过标签配置的属性，未定义则不支持外部配置

                    P103DemoComponentP3d4.prototype.$STATE_KEYS = ["$SLOT"]; // 可更新的state属性，未定义则不支持外部更新state
                    // 组件函数

                    function P103DemoComponentP3d4() {
                        var _this2 = this;
                        ...
                        this.$actions = {
                            btn1Click: function btn1Click(e) {
                                _this2.$state.cnt1++;
                                _this2.$state.msg = "点击按钮1，共 " + _this2.$state.cnt1 + " 次";

                                _this2.render();
                            },
                            btn2Click: function btn2Click(e) {
                                _this2.$state.cnt2++;
                                _this2.$state.msg = "点击按钮2，共 " + _this2.$state.cnt2 + " 次";

                                _this2.render();
                            },
                            btn3Click: function btn3Click(e) {
                                var sec = 5;

                                var btn3 = _this2.getRefComponent("btn3");
                    ...
                    P103DemoComponentP3d4.prototype.nodeTemplate = function nodeTemplate($state, $options, $actions, $this) {
                        return {
                            t: "div",
                            r: 1,
                            k: 10,
                    ...
                    P106DemoSlotP6d2.prototype.$OPTION_KEYS = undefined; // 可通过标签配置的属性，未定义则不支持外部配置

                    P106DemoSlotP6d2.prototype.$STATE_KEYS = ["$SLOT"]; // 可更新的state属性，未定义则不支持外部更新state
                    // 组件函数

                    function P106DemoSlotP6d2() {
                        var options = arguments.length > 0 && arguments[0] !== undefined ? arguments[0] : {};
                        // 组件默认选项值
                        this.$options = {}; // 组件默认数据状态值
                    ...
                    P106DemoSlotP6d2.prototype.nodeTemplate = function nodeTemplate($state, $options, $actions, $this) {
                        return {
                            t: "demo-slot-component-default",
                            r: 1,
                            m: 1,
                            k: 2,
                            c: [
                    ...
                    function $rpose$buildin$Router() {
                        ...
                        this.init = function() {
                            rpose.router.register({
                                path: _this3.$options.path,
                                component: _this3,
                                default: !!_this3.$options.default,
                                notfound: !!_this3.$options.notfound
                            });

                            _this3.route = function(ctx) {
                    ...
                    // 组件 P107DemoRouteP7d4
                    // 注:应通过rpose.newComponentProxy方法创建组件代理对象后使用，而不是直接调用方法或用new创建
                    // ------------------------------------------------------------------------------------------------------
                    // 属性接口定义

                    P107DemoRouteP7d4.prototype.$OPTION_KEYS = undefined; // 可通过标签配置的属性，未定义则不支持外部配置

                    P107DemoRouteP7d4.prototype.$STATE_KEYS = ["$SLOT"]; // 可更新的state属性，未定义则不支持外部更新state
                    // 组件函数

                    function P107DemoRouteP7d4() {
                        var options = arguments.length > 0 && arguments[0] !== undefined ? arguments[0] : {};
                        // 组件默认选项值
                        this.$options = {}; // 组件默认数据状态值
                    ...
                    P107DemoRouteP7d4.prototype.nodeTemplate = function nodeTemplate($state, $options, $actions, $this) {
                        return {
                            t: "div",
                            r: 1,
                            k: 24,
                    ...
                    rpose.mount(rpose.newComponentProxy("p107-demo-route-p7d4").render(), "body");
                })(rpose.$$);
            },
            {
