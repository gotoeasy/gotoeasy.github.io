...
(function($$, escapeHtml) {
    // 组件注册
    rpose.registerComponents({ "ui-helloworld": UiHelloworld, "demo-2-4-helloworld": Demo24Helloworld });
    ...
    // 属性接口定义
    UiHelloworld.prototype.$OPTION_KEYS = null; // 可通过标签配置的属性，未定义则不支持外部配置
    UiHelloworld.prototype.$STATE_KEYS = ["name"]; // 可更新的state属性，未定义则不支持外部更新state

    // 组件函数
    function UiHelloworld(options = {}) {
        // 组件默认选项值
        this.$options = {};
    ...
    UiHelloworld.prototype.nodeTemplate = function nodeTemplate($state, $options, $actions, $this) {
        let name = $state.name;
        return { r: 1, t: "span", c: [{ s: "hello " + escapeHtml(name) + "!", k: 2 }], x: 1, k: 1 };
    };
    ...
    // 属性接口定义
    Demo24Helloworld.prototype.$OPTION_KEYS = null; // 可通过标签配置的属性，未定义则不支持外部配置
    Demo24Helloworld.prototype.$STATE_KEYS = null; // 可更新的state属性，未定义则不支持外部更新state

    // 组件函数
    function Demo24Helloworld(options = {}) {
        // 组件默认选项值
        this.$options = {};
     ...
     * 节点模板函数
     */
    Demo24Helloworld.prototype.nodeTemplate = function nodeTemplate($state, $options, $actions, $this) {
        return {
            r: 1,
            t: "div",
            c: [
                { t: "input", e: { change: $actions.fnChange }, a: { class: { _wok2n: 1 } }, x: 1, k: 2 },
                { t: "ui-helloworld", a: { ref: "hello", name: "rpose", $context: $this }, m: 1, k: 3 }
            ],
            x: 1,
            k: 1
        };
    ...
    // 组件挂载
    rpose.mount(rpose.newComponentProxy("demo-2-4-helloworld").render(), "body");
