            ...
            function(require, module, exports) {
                ...
                (function($$) {
                    ...
                    rpose.registerComponents({
                        "demo-slot-component": DemoSlotComponent,
                        "p106-demo-slot-p6d1": P106DemoSlotP6d1
                    }); // ------------------------------------------------------------------------------------------------------
                    // 组件 DemoSlotComponent
                    // 注:应通过rpose.newComponentProxy方法创建组件代理对象后使用，而不是直接调用方法或用new创建
                    // ------------------------------------------------------------------------------------------------------
                    // 属性接口定义

                    DemoSlotComponent.prototype.$OPTION_KEYS = undefined; // 可通过标签配置的属性，未定义则不支持外部配置

                    DemoSlotComponent.prototype.$STATE_KEYS = ["$SLOT"]; // 可更新的state属性，未定义则不支持外部更新state
                    // 组件函数

                    function DemoSlotComponent() {
                    ...
                    DemoSlotComponent.prototype.nodeTemplate = function nodeTemplate($state, $options, $actions, $this) {
                        var v_Array = [];
                        var slotVnodes_1qds6e1 = [],
                            slotVnodes_1ssgsoq = [];
                        ($state.$SLOT || []).forEach(function(vn) {
                            if (vn.a) {
                                vn.a.slot === "my-slot1" && (slotVnodes_1qds6e1 = vn.c || []);
                                vn.a.slot === "my-slot2" && (slotVnodes_1ssgsoq = vn.c || []);
                            }
                        });
                        v_Array.push({
                            t: "ul",
                            ...
                            c: [
                                {
                                    ...
                                    k: 1,
                                    c: (function(_Ary) {
                                        _Ary.push.apply(_Ary, _toConsumableArray(slotVnodes_1qds6e1));
                                ...
                                },
                                {
                                    ...
                                    k: 2,
                                    c: (function(_Ary) {
                                        _Ary.push.apply(_Ary, _toConsumableArray(slotVnodes_1ssgsoq));
                    ...
                    P106DemoSlotP6d1.prototype.$OPTION_KEYS = undefined; // 可通过标签配置的属性，未定义则不支持外部配置

                    P106DemoSlotP6d1.prototype.$STATE_KEYS = ["$SLOT"]; // 可更新的state属性，未定义则不支持外部更新state
                    // 组件函数

                    function P106DemoSlotP6d1() {
                        var options = arguments.length > 0 && arguments[0] !== undefined ? arguments[0] : {};
                        // 组件默认选项值
                        this.$options = {}; // 组件默认数据状态值
                    ...
                    P106DemoSlotP6d1.prototype.nodeTemplate = function nodeTemplate($state, $options, $actions, $this) {
                        return {
                            t: "demo-slot-component",
                            r: 1,
                            m: 1,
                            k: 7,
                            c: [
                    ...
                    rpose.mount(rpose.newComponentProxy("p106-demo-slot-p6d1").render(), "body");
                })(rpose.$$);
            },
            {
