...
(function($$, escapeHtml) {
    // 组件注册
    rpose.registerComponents({ "ui-helloworld": UiHelloworld, "demo-2-3-helloworld": Demo23Helloworld });
    ...
    // 属性接口定义
    Demo23Helloworld.prototype.$OPTION_KEYS = null; // 可通过标签配置的属性，未定义则不支持外部配置
    Demo23Helloworld.prototype.$STATE_KEYS = null; // 可更新的state属性，未定义则不支持外部更新state

    // 组件函数
    function Demo23Helloworld(options = {}) {
        // 组件默认选项值
        this.$options = {};
     ...
     * 节点模板函数
     */
    Demo23Helloworld.prototype.nodeTemplate = function nodeTemplate($state, $options, $actions, $this) {
        return {
            r: 1,
            t: "div",
            c: [
                { t: "input", e: { change: $actions.fnChange }, a: {}, x: 1, k: 2 },
    ...
    // 组件挂载
    rpose.mount(rpose.newComponentProxy("demo-2-3-helloworld").render(), "body");
