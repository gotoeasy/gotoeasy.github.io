...
// ------------------------------这批变量允许被配置------------------------------
var DEBUG;							// 是否调试模式
var IS_REMOVE_ATTR_BIND;			// 是否最终要删除数据绑定声明
...
var EVENT_CHECK_MEMORY = '_eventCheckMap';					// 页面节点上保存模板ID用的属性名，模板未绑定时自动算出模板ID并输出

function set(data, key, value, forceUpdate){
/*del*/ info("调用set进行赋值", key);
...
/*del*/ info("开始内存脏数据清除");
	var keys = [];
	for (var k in _mapDataIdDataObject){
		keys.push(k);
	}
	for(var i=0; i< keys.length; i++){
		checkMapDataId(keys[i]);
	}
}
...
function checkMapDataId(dataid){
	async(function(){
		if(!document.querySelector("." + dataid)){
			delete _mapDataIdDataObject[dataid];
/*del*/ log("清除不再使用的数据：" + dataid);
		}
	},100); // 优先度低慢慢做就行
}
	...
	if (isObject(data)){
		...
		if (isPlainObject(data)){
			for (var k in data ){
				...
				defineDataProperty(data, k, data[k]);
			}
		}else if (isArray(data)){
			for (var i=0; i<data.length; i++ ){
				observe(data[i], data, i);
				defineDataProperty(data, i, data[i]);
			}
		...
		defineDataProperty(parent, key, data); // 确保key属性被监视
	}
	...
	if (cssFields){
		for (var key in bindInfo){
			if (key != S_BIND_INFO_PROP_DATA_ID && key != BIND_KEY_FIELD && key != BIND_KEY_CLICK){	// TODO 排除页面刷新无关的FIELD和CLICK
				...
				if (key == BIND_KEY_STYLE){ // TODO 硬编码改善
				...
				}else if (key == BIND_KEY_CLASS){ // TODO 硬编码改善
					var kvs = bindInfo[key].split(';'); // data-bind="class:hide=hide;color='info-color';bg-color=getBgColor();"
					for (var i=0, kv; i<kvs.length; i++){
						kv = kvs[i].split('=');

						if (kv.length == 1){ // 特殊处理 data-bind="class:fieldClass"，class完全设定为fieldClass所指定的值
							var fields = [];
...
function defineElement(el, data){
	...
	dataid && addClass(el, dataid); // 写入当前数据对象ID，用于清除未使用数据
...
function installRenders(){
	...
	putRender(BIND_KEY_CLASS, function(el, data, bindText) {
		...
		if (data[bindText]){
			// 特殊处理 data-bind="class:fieldClass"，class完全设定为fieldClass所指定的值
			setClass(el, data[bindText] );
			return;
		}
...
function setClass(el, cls){
	...
	if ($isIE){
		...
		for (var i=0; i<el.classList.length; i++){
			if (!startsWith(el.classList[i], UID_PREFIX_DATA)){ // 数据ID前缀的字段名类除外
				delNames.push(el.classList[i]);
			}
		}
		for (var i=0; i<delNames.length; i++){
			el.classList.remove(delNames[i]);
		}
	}

	// 添加
	for (var i=0; i<addNames.length; i++){
		addClass(el, addNames[i]);
	}
...
function on(eventNames, fn) {
    if (eventNames && isFunction(fn)) {
		eventNames.replace(REG_EVENT, function(name, pos) {
			var isExist = false, fns = _callbacks[name] = _callbacks[name] || [];
			...
			!isExist && fns.push(fn); // 相同函数在同一事件中不让重复绑定
		});
    }
};
...
function trigger(eventNames, eventObj){
	...
	eventNames.replace(REG_EVENT, function(name) {
		...
		for (var i=0;i<fns.length ;i++ ){
			var fn = fns[i];
			fn.once && off(name, fn); // 仅执行一次的事件函数，先关闭掉后执行
			tryApply(eventObj, fn, args);
		}
	});
};
...
/*del*/	log('当前渲染器', _renderMap);
	...
	// 删除绑定属性声明
	removeDataBindAttr(dom);

/*del*/	info('bind耗时', new Date()-st, 'ms');
}
