...
function initOptionValues(){
	...
	DOM_ATTR_BIND = Options.domAttrBind || 'data-bind';							// 控件的数据绑定属性名
	SELECTOR_DATA_BIND = S_LEFT_ZKH + DOM_ATTR_BIND + S_RIGHT_ZKH;				// data-bind选择器

	DATA_KEY_FN_ID = Options.dataKeyFnId || '_$id';								// 取数据的ID的方法名
	DATA_KEY_FN_PARENT = Options.dataKeyFnParent || '_$parent';					// 取父数据对象的方法名
	DATA_KEY_FN_ROOT = Options.dataKeyFnRoot || '_$root';						// 取根数据对象的方法名
	DATA_KEY_FN_DATA = Options.dataKeyFnData || '_$data';						// 取本数据对象的方法名

	BIND_REF_ROOT = Options.bindRefRoot || '$root';								// 定义指向根数据对象的关键字
	BIND_REF_PARENT = Options.bindRefParent || '$parent';						// 定义指向父数据对象的关键字
	BIND_REF_DATA = Options.bindRefData || '$data';								// 定义指向本数据对象的关键字

	BIND_KEY_FIELD = Options.bindKeyField || 'field';							// 绑定键，用于指定回写数据的字段
	BIND_KEY_VALUE = Options.bindKeyValue || 'value';							// 绑定键，用于绑定value
	BIND_KEY_INNERTEXT = Options.bindKeyInnerText || 'text';						// 绑定键，用于绑定InnerText
	BIND_KEY_INNERHTML = Options.bindKeyInnerHtml || 'html';						// 绑定键，用于绑定InnerHtml
	BIND_KEY_OPTIONS = Options.bindKeyOptions || 'options';						// 绑定键，用于绑定下拉框的option选项
	BIND_KEY_READONLY = Options.bindKeyReadonly || 'readonly';					// 绑定键，用于绑定readonly
	BIND_KEY_DISABLED = Options.bindKeyDisabled || 'disabled';					// 绑定键，用于绑定disabled
	BIND_KEY_VISIBLE = Options.bindKeyVisible || 'visible';						// 绑定键，用于绑定控件是否可见
	BIND_KEY_CHECKED = Options.bindKeyChecked || 'checked';						// 绑定键，用于绑定复选框和单选框的选中状态
	BIND_KEY_STYLE = Options.bindKeyStyle || 'style';							// 绑定键，用于绑定style
	BIND_KEY_CLASS = Options.bindKeyClass || 'class';							// 绑定键，用于绑定class
//	BIND_KEY_TEMPLATE = Options.bindKeyTemplate || 'template';	// 绑定键，用于绑定template
	BIND_KEY_FOREACH = Options.bindKeyForeach || 'foreach';						// 绑定键，用于绑定数组循环
	BIND_KEY_WITH = Options.bindKeyWith || 'with';								// 绑定键，用于绑定对象作用域
	BIND_KEY_IF = Options.bindKeyIf || 'if';										// 绑定键，用于控制子节点是否显示
	BIND_KEY_CLICK = Options.bindKeyClick || 'click';							// 绑定键，用于绑定点击事件

	EVENT_DATA_CHAGE = Options.eventDataChage || 'datachange';					// 定义数据变化的事件名
	EVENT_UPDATE_VIEW = Options.eventUpdateView || 'updaueview';					// 定义更新视图的事件名

	UID_PREFIX_DATA = Options.uidPrefixData || 'd';								// 定义数据UID使用的前缀
	UID_PREFIX_TEMPLATE = Options.uidPrefixTemplate || 't';						// 定义模板UID使用的前缀
}
...
function parseBindInfo(el){
	...
	var tmpTxt = bindText.replace(/,{1}\s*\w+\s*:{1}/g, function(match){  return '\n' + match.substring(1);	});	// 逗号+任意空格+类字母+任意空格+冒号  ->  【换行】+任意空格+类字母+任意空格+冒号
	kvs = tmpTxt.split('\n');										// 按【换行】区分不同的绑定
...
function datachangeEventListener(e){
	...
	if (field == null){
		log('[ignore datachange]', el);  // TODO 有必要整个构建工具把log输出语句干掉？
		return;
	}
	var data = getData(bindInfo[S_BIND_INFO_PROP_DATA_ID]);
